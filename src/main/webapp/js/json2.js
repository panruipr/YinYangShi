...
(function($) {
    ...
    $.fn.bootstrapValidator.validators.idCardNo = {
    	validate:function(validator, $field, options){
    		var idCard = $field.val();
    		var pId = idCard.replace(/\s+/g,"");//去掉所有空格
    		pId = pId.toUpperCase();//转大写
    		var arrVerifyCode = [1,0,"X",9,8,7,6,5,4,3,2];//校验码
    	    var Wi = [7,9,10,5,8,4,2,1,6,3,7,9,10,5,8,4,2];//Wi:表示第i位置上的加权因子
